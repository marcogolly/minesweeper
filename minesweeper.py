...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0]-1, cell[0]+2):
          for j in range(cell[1]-1, cell[1]+2):
            if (i != cell[0] or j != cell[1]) and i>=0 and j>=0 and i<8 and j<8:
              neighbors.add((i,j))
    ...
    def update_safe_cells(self):
      ...
      for a in self.knowledge:
        ...
        if a.known_safes():
          self.safes =self.safes.union(a.known_safes())
        if a.known_mines():
          self.mines =self.mines.union(a.known_mines())
        a.cells-=self.moves_made
  ...
  #todo
    ...
    def make_random_move(self):
        ...
        """
        self.update_safe_cells()
        if len(self.mines) + len(self.moves_made) ==64:
          return None
        while True:
